...
# Form implementation generated from reading ui file 'single_root.ui'
#
...
class Ui_single(object):
    ...
    def open_method_type(self):
        if self.radioButton_2.isChecked():
            self.comboBox.clear()
            self.comboBox.addItems(["Secant","Newton Raphson","Fixed Points"])
            
    def bracket_method_type(self):
        if self.radioButton.isChecked():
            self.comboBox.clear()
            self.comboBox.addItems(["Bisection","False Position"])
        
## action of ok button to move to output window need here to call algorithms depend on radio button
    def okAction(self):
